...
// --- CRITICAL FIX: Include Map Implementation ---
...
// --- LINKER FIX: ACTUALLY DEFINE THE STATE HERE ---
...
void net_send_input(float fwd, float strafe, int jump, int crouch, int shoot, int reload) {
...
void render_gun(PlayerState *me) {
    ...
    glColor3f(0.4f, 0.4f, 0.45f); glBegin(GL_QUADS);
...
int main(int argc, char* argv[]) {
    ...
    SDL_Window *win = SDL_CreateWindow("SHANK PIT HYBRID CLIENT v27", 100, 100, 1280, 720, SDL_WINDOW_OPENGL);
    ...
    while(running) {
        ...
        if (app_state == STATE_LOBBY) {
            ...
            if (app_state == STATE_GAME_NET) {
                net_update();
                net_send_input(fwd, str, jump, crouch, shoot, reload);
            } else {
                int wpn = -1;
                if(k[SDL_SCANCODE_1]) wpn = WPN_KNIFE;
                if(k[SDL_SCANCODE_2]) wpn = WPN_MAGNUM;
                local_update(0.016f, fwd, str, cam_yaw, cam_pitch, shoot, wpn, jump, crouch, reload);
            }
            render_game();
        }
