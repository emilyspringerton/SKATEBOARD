...
void net_send_input(float fwd, float strafe, int jump, int crouch, int shoot, int reload, int weapon) {
    ...
    in->weapon_req = weapon; // Send request
...
void render_gun(PlayerState *me) {
    ...
    glTranslatef(0.3f, -0.3f + bob, -0.5f + recoil); 
    glRotatef(180.0f, 0, 1, 0);

    // Dynamic Gun Model
    if (me->current_weapon == WPN_KNIFE) {
        glColor3f(0.6f, 0.6f, 0.6f); 
        glScalef(0.3f, 0.3f, 1.0f); // Short Blade
    } else if (me->current_weapon == WPN_MAGNUM) {
        glColor3f(0.7f, 0.7f, 0.7f); 
        glScalef(0.5f, 0.5f, 1.0f);
    } else if (me->current_weapon == WPN_AR) {
        glColor3f(0.1f, 0.1f, 0.1f);
        glScalef(0.4f, 0.4f, 2.0f); // Long barrel
    } else if (me->current_weapon == WPN_SHOTGUN) {
        glColor3f(0.4f, 0.2f, 0.1f);
        glScalef(0.6f, 0.4f, 1.5f); // Thick
    } else if (me->current_weapon == WPN_SNIPER) {
        glColor3f(0.2f, 0.3f, 0.2f);
        glScalef(0.3f, 0.3f, 2.5f); // Very long
    }
...
int main(int argc, char* argv[]) {
    ...
    SDL_Window *win = SDL_CreateWindow("SHANK PIT ARSENAL v28", 100, 100, 1280, 720, SDL_WINDOW_OPENGL);
    ...
    while(running) {
        ...
        if (app_state == STATE_LOBBY) {
            ...
            draw_text("[A] JOIN LIVE SERVER (s.farthq.com)", 50, 130, 20.0f, 0, 1, 0);
            draw_text("[D] LOCAL DEMO (OFFLINE)", 50, 170, 20.0f, 0, 1, 1);
            ...
            if (app_state == STATE_GAME_NET) {
                net_update();
                net_send_input(fwd, str, jump, crouch, shoot, reload, weapon);
            } else {
