...
void draw_char(char c, float x, float y, float s) {
    ...
    // Thicker text for Cyberpunk feel
...
void draw_string(const char* str, float x, float y, float size) { while(*str) { draw_char(*str, x, y, size);
x += size * 1.5f; str++; } }

#define MAX_TRAILS 4096 
#define GRID_SIZE 50.0f
typedef struct { int cx, cz; float life;
} Trail;
...
void draw_gun_model(int weapon_id) {
    switch(weapon_id) {
        case WPN_KNIFE:   glColor3f(0.8f, 0.8f, 0.9f);
        glScalef(0.05f, 0.05f, 0.8f); break;
        ...
        case WPN_SHOTGUN: glColor3f(0.5f, 0.3f, 0.2f);
        glScalef(0.25f, 0.15f, 0.8f); break;
...
void draw_hud(PlayerState *p) {
    ...
    glColor3f(0, 1, 0);
    if (current_fov < 50.0f) { glBegin(GL_LINES); glVertex2f(0, 360); glVertex2f(1280, 360); glVertex2f(640, 0); glVertex2f(640, 720);
    glEnd(); } 
    ...
    // --- HIT INDICATORS ---
    if (p->hit_feedback > 0) {
        if (p->hit_feedback >= 25) glColor3f(1.0f, 0.0f, 0.0f);
        // RED (Kill/High Dmg)
        ...
        if (p->hit_feedback >= 25) {
            draw_circle(640, 360, 28.0f, 16);
            // Outer Kill Ring
        }
    }

    glColor3f(0.2f, 0, 0);
    glRectf(50, 50, 250, 70); glColor3f(1.0f, 0, 0);
    glRectf(50, 50, 50 + (p->health * 2), 70);
    glColor3f(0, 0, 0.2f);
    glRectf(50, 80, 250, 100); glColor3f(0.2f, 0.2f, 1.0f);
...
void draw_scene(PlayerState *render_p) {
    ...
    float cam_y = render_p->in_vehicle ?
    8.0f : (render_p->crouching ? 2.5f : EYE_HEIGHT);
...
int main(int argc, char* argv[]) {
    ...
    while(running) {
        ...
        while(SDL_PollEvent(&e)) {
            ...
            if (app_state == STATE_LOBBY) {
                if(e.type == SDL_KEYDOWN) {
                    if (e.key.keysym.sym == SDLK_d) { app_state = STATE_GAME_LOCAL;
                    local_init_match(1, MODE_DEATHMATCH); }
                    if (e.key.keysym.sym == SDLK_b) { app_state = STATE_GAME_LOCAL;
                    local_init_match(8, MODE_DEATHMATCH); }
                    if (e.key.keysym.sym == SDLK_k) { app_state = STATE_GAME_LOCAL;
        ...
        if (app_state == STATE_LOBBY) {
            ...
            float target_fov = (local_state.players[0].current_weapon == WPN_SNIPER && (SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_RIGHT))) ?
            20.0f : 75.0f;
